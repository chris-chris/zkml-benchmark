...
pub fn create_mlp_circuit(input_size: usize) -> Vec<CircuitGate<Fp>> {
    let mut gates = vec![];
    ...
    // for _ in 0..input_size {
    //     let r = gates_row.next().unwrap();
    //     let weight = Fp::from(2u32); // 가중치 값 (임의 설정)
    //     let bias = Fp::from(2u32);   // 바이어스 값 (임의 설정)
...
pub fn fill_in_mlp_witness<F: FftField>(
    ...
    // // 은닉 레이어 2: 선형 변환과 ReLU 적용
    // for _ in 0..input_size {
    //     let r = witness_row.next().unwrap();

    //     let input_val = witness[1][r]; // 은닉 레이어 1의 출력값
    //     let weight = F::from(2u32);    // 가중치 값
    //     let bias = F::from(2u32);      // 바이어스 값
    ...
    //     witness[2][r] = relu_output;    // ReLU 결과를 witness에 저장
    // }
